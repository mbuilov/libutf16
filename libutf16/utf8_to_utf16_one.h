...
/* read one unicode character from utf8 string, returns:
  ...
  (size_t)-3 - first part of utf16 surrogate pair of unicode character has been read,
               need to repeat the call to read second part of the surrogate pair;
  >0         - number of bytes consumed from s, state has been reset to zero,
               one utf16 character (or the second part of surrogate pair) has been read;
...
/* determine the length of one unicode character encoded in utf8 string, returns:
  (size_t)-1 - if s contains invalid utf8 byte sequence;

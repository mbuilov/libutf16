...
/* convert utf8 0-terminated string to utf16 0-terminated one,
 ...
 returns number of stored utf16_char_t's, including terminating 0:
  ...
  > sz  - output buffer is too small, return value is the required buffer size to store
   ...
   and stored in output buffer, including 0-terminator, in utf16_char_t's;
 - on success:
 ...
 - if input utf8 string is invalid:
  (*q) - points beyond the last valid utf8_char_t, if output buffer is too small,
   it may be different from the last successfully converted utf8_char_t,
   the last valid utf8_char_t is _not_ 0;
  (*b) - if sz > 0, points beyond the last stored (non-0) utf16_char_t */
...
/* convert 'n' utf8_char_t's to utf16 ones,
 ...
 - on success:
  ...
  (*b) - points beyond the last converted utf16_char_t stored in output buffer;
 - if output buffer is too small:
